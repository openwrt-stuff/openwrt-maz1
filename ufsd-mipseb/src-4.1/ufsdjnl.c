...
//
static const char s_FileVer[] = "$Id: ufsdjnl.c 214002 2013-10-08 14:08:41Z shura $";
...
//  hdr->start = hdr->end = ( j->tail - bh->b_blocknr ) << j->blocksize_bits;
  if ( 0 == j->tail ) {
    ...
    hdr->start  = ( j->head - bh->b_blocknr ) << j->blocksize_bits;
    hdr->end    = ( j->tail - bh->b_blocknr ) << j->blocksize_bits;
  ...
  jnl_debug( JNL_TRACE_LEVEL_HDR, "marking dirty header buffer b=%"PSCT"x\n", bh->b_blocknr );
...
//  trace_jnl__start_commit( journal, commit_transaction );
  ...
  // This is consistent with the existing behaviour that multiple jnl_get_write_access() calls to the same
...
//      jnl__buffer_abort_trigger( jh, jh->frozen_data ? jh->frozen_triggers : jh->triggers );
    ...
    if ( NULL != find_revoke_record( j, revoke, bh->b_blocknr ) ) {
      ...
      if ( unlikely( bh->b_size != blocksize ) ) {
        J_ASSERT( 512 == bh->b_size );
        bi->bnum  = (unsigned long long)bh->b_blocknr >> (j->blocksize_bits-9);
        off       = 0;
//        test_tail = 1;
      } else {
      ...
      bi->cksum = ~CalcCheckSumEx( addr + off, blocksize, chksum );
      atomic_kunmap( addr );
    }
    ...
    blhdr->num_blocks += 1;
    bi  += 1;
    chksum = 0; // TODO merge several bi
...
// jnl_proc_info
//
// Add/Remove /proc/fs/ufsd_jnl/..
///////////////////////////////////////////////////////////
static void
jnl_proc_info_create(
    IN jnl* j
    )
{
  struct proc_dir_entry* e = proc_mkdir( j->devname, proc_info_root );
  if ( NULL == e )
    printk( KERN_NOTICE "jnl: cannot create /proc/%s/%s", proc_info_root_name, j->devname );
  else {
...
///////////////////////////////////////////////////////////
// jnl_proc_info_delete
//
// Remove /proc/fs/ufsd_jnl/..
///////////////////////////////////////////////////////////
static void
jnl_proc_info_delete(
    IN jnl* j
    )
{
  if ( NULL != proc_info_root ) {
    remove_proc_entry( j->devname, proc_info_root );
    j->procdir = NULL;
  }
}

#else
  #define jnl_proc_info_create( j )
  #define jnl_proc_info_delete( j )
...
//  j->flags |= JNL_FLAGS_LOADED;
  ...
  *journal = j;
  ...
  // Create /proc/fs/ufsd_jnl/..
  jnl_proc_info_create( j );
